# Appendices - Vital Records FHIR Messaging (VRFM) IG v3.0.0-Preview1
...
1. Software for creating the following types of messages: Death Record Submission, Death Record Update, Death Record Void, and Acknowledgement
1. Software for parsing the following types of messages: Coding Response, Extraction Error Response, Coding Error Response, and Acknowledgement
1. Software to implement message transport and the message processing algorithms shown above
1. Persistent logging infrastructure for the`[message log]`component. This is used to record: a. Received messages (for duplication elimination and audit purposes) b. Successfully delivered messages (for audit purposes) c. Unsuccessfully delivered messages (for audit purposes)
1. Persistent queue infrastructure for the`[pending queue]`component
